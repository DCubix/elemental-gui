...
#if __cplusplus > 0
#define SMOL_INIT(type)                                                                            \
    type {}
#define SMOL_OBJ(type) type
#else
#define SMOL_INIT(type)                                                                            \
    (type) {                                                                                       \
        0                                                                                          \
...
#define SMOL_FRAME_EVENT_LOOP(frame, it)                                                           \
    for (smol_frame_event_t it; smol_frame_acquire_event(frame, &it);)
...
#elif defined(__EMSCRIPTEN__)
typedef int smol_i32;
typedef long long esmol_i64;
typedef unsigned long long smol_u64;
#else
typedef int smol_i32;
typedef long int smol_i64;
typedef unsigned long int smol_u64;
#endif

#if _WIN64 || __linux__
#ifndef SMOL_SIZE_T
#define SMOL_SIZE_T
typedef smol_u64 smol_size_t;
#endif
#ifndef SMOL_SSIZE_T
#define SMOL_SSIZE_T
typedef smol_i64 smol_ssize_t;
#endif
#else
#ifndef SMOL_SIZE_T
#define SMOL_SIZE_T
typedef smol_u32 smol_size_t;
#endif
#ifndef SMOL_SSIZE_T
#define SMOL_SSIZE_T
typedef smol_i32 smol_ssize_t;
#endif
#endif

#ifndef SMOL_ALLOCATOR
typedef struct _smol_allocator_t smol_allocator_t;
#endif
...
// - int height           -- height of the window
// - const char* title    -- title of the window
// - unsigned int         -- flags of the created window
//Returns: smol_frame_t*  -- a handle to the newly created window
smol_frame_t* smol_frame_create_flags(int width, int height, const char* title, unsigned int flags);

//smol_frame_create - Maximizes or restores a window
//Arguments:
//smol_frame_t* frame -- A pointer to the frame
//int maximize        -- A boolean to toggle maximization on/off
void smol_frame_set_maximized(smol_frame_t* frame, int maximize);

//smol_frame_set_default_cursor - Sets the frame cursor to one of the default cursors
//Arguments:
//smol_frame_t* frame -- A pointer to the frame
//unsigned int cursor -- A default cursor ID
void smol_frame_set_default_cursor(smol_frame_t* frame, unsigned int cursor_id);
...
//smol_frame_set_cursor_position - Sets the mouse cursor position on screen
//Arguments:
// - smol_frame_t* -- A frame within the cursor is being placed, can be NULL, position will be absolute in desktop space
// - int x - Horizontal coordinate
// - int y - Vertical coordinate
void smol_frame_set_cursor_position(smol_frame_t* frame, int x, int y);
...
smol_frame_gl_spec_t smol_init_gl_spec(
    int major_version,
    int minor_version,
...
#ifdef SMOL_USE_VULKAN
//smol_frame_create_vulkan_surface - Creates VkSurfaceKHR for vulkan swapchain presentation
//Arguments
// - smol_frame_t* frame -- a frame the surface is created for
// - VkInstance instance -- The vulkan instance handle
// - VkAllocationCallbacks* alloc_b -- A pointer to the vulkan allocation callbacks (optional, NULL = default callbacks)
// - VkResult* result -- A pointer to VkResult structure (optional, NULL = error won't be handled, on failure VK_NULL_HANDLE surface is returned)
VkSurfaceKHR smol_frame_create_vulkan_surface(
    smol_frame_t* frame,
    VkInstance instance,
...
#ifndef SMOL_ALLOCATOR
typedef struct _smol_allocator_t {
    void* (*allocate)(smol_size_t size, void* user);
    void* (*reallocate)(void* mem, smol_size_t size, void* user);
    void (*release)(void* memory, void* user);
    void* user;
} smol_allocator_t;
#endif

#ifndef SMOL_DEFAULT_ALLOCATOR
#define SMOL_ALLOCATOR_IMPLS_NEEDED
#define SMOL_DEFAULT_ALLOCATOR
void* smol_std_alloc(smol_size_t size, void* user);
void* smol_std_realloc(void* mem, smol_size_t size, void* user);
void smol_std_release(void* mem, void* user);
#define SMOL_STD_ALLOCATOR                                                                         \
    SMOL_OBJ(smol_allocator_t) {                                                                   \
        smol_std_alloc, smol_std_realloc, smol_std_release, NULL                                   \
    }
#endif
...
typedef enum {
    ...
    SMOL_FRAME_CONFIG_ALWAYS_ON_TOP = 0x00000010U,
    SMOL_FRAME_CONFIG_BORDERLESS = 0x00000020U,
...
typedef enum {
    ...
    SMOL_CURSOR_GRAB_X,
    SMOL_CURSOR_GRAB_Y,
    SMOL_CURSOR_GRAB_X_ASCEND,
    SMOL_CURSOR_GRAB_X_DESCEND,
    SMOL_CURSOR_GRAB_XY,
    SMOL_CURSOR_COUNT
} smol_default_cursor_id;
...
#define SMOL_ASSERT(condition)                                                                     \
    if (!(condition))                                                                              \
    puts("SMOL FRAME ASSERTION FAILED!\n"                                                          \
         "CONDITION: " #condition "\n"                                                             \
         "IN FILE: '" __FILE__ "'\n"                                                               \
...
#ifdef SMOL_ALLOCATOR_IMPLS_NEEDED
void* smol_def_alloc(smol_size_t size, void* user) {
    (void)user;
    return malloc(size);
}
void* smol_def_realloc(void* mem, smol_size_t size, void* user) {
    (void)user;
    return realloc(mem, size);
}
void smol_def_release(void* mem, void* user) {
    free(mem);
}
void* smol_allocator_alloc(smol_allocator_t* alloc, smol_size_t size) {
    return alloc->allocate(size, alloc->user);
}
void* smol_allocator_realloc(smol_allocator_t* alloc, void* mem, smol_size_t size) {
    return alloc->reallocate(mem, size, alloc->user);
}
void smol_allocator_free(smol_allocator_t* alloc, void* mem) {
    alloc->release(alloc, mem);
}
#undef SMOL_ALLOCATOR_IMPLS_NEEDED
#endif
...
typedef int smol_StretchDIBits_proc(
    HDC hdc,
    int xDest,
    int yDest,
    int DestWidth,
...
smol_wglCreateContextAttribsARB_proc(HDC hDC, HGLRC hshareContext, const int* attribList);

typedef BOOL APIENTRY smol_wglChoosePixelFormatARB_proc(
    HDC hdc,
...
typedef BOOL APIENTRY smol_wglGetPixelFormatAttribivARB_proc(
    HDC hdc,
    int iPixelFormat,
    int iLayerPlane,
    UINT nAttributes,
...
typedef BOOL APIENTRY smol_wglGetPixelFormatAttribfvARB_proc(
    HDC hdc,
    int iPixelFormat,
    int iLayerPlane,
    UINT nAttributes,
...
static HCURSOR smol__cursor[SMOL_CURSOR_COUNT];

LRESULT CALLBACK smol_frame_handle_event(HWND wnd, UINT msg, WPARAM wParam, LPARAM lParam);

#ifdef SMOL_FRAME_MULTITHREADED

static HANDLE smol__frame_thread;
static HANDLE smol__frame_thread_should_exit;
static HWND smol__root_window;
static volatile UINT smol__window_count;

#define WM_SMOL_CREATE_WINDOW WM_USER + 1
#define WM_SMOL_SET_WINDOW_TITLE WM_USER + 2
#define WM_SMOL_DESTROY_WINDOW WM_USER + 3
#define WM_SMOL_IS_WINDOW WM_USER + 4
#define WM_SMOL_SET_WINDOW_POS WM_USER + 5

DWORD WINAPI smol_frame_thread(LPVOID lpParameter);
LRESULT CALLBACK smol_root_window_event_handler(HWND wnd, UINT msg, WPARAM wParam, LPARAM lParam);
HWND smol_CreateWindow(
    DWORD dwExStyle,
    LPCWSTR lpClassName,
...
BOOL smol_DestroyWindow(HWND window);
BOOL smol_SetWindowText(HWND window, LPCWSTR title);
BOOL smol_IsWindow(HWND window);
#else
#define smol_CreateWindow CreateWindowEx
#define smol_DestroyWindow DestroyWindow
#define smol_SetWindowText SetWindowText
#define smol_IsWindow IsWindow
#define smol_SetWindowPos SetWindowPos
#endif
...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
    if (smol__wingdi == NULL) {
        ...
        smol_GetStockObject =
            (smol_GetStockObject_proc*)GetProcAddress(smol__wingdi, "GetStockObject");
        smol_ChoosePixelFormat =
            (smol_ChoosePixelFormat_proc*)GetProcAddress(smol__wingdi, "ChoosePixelFormat");
        smol_SetPixelFormat =
            (smol_SetPixelFormat_proc*)GetProcAddress(smol__wingdi, "SetPixelFormat");
        smol_SwapBuffers = (smol_SwapBuffers_proc*)GetProcAddress(smol__wingdi, "SwapBuffers");
        smol_StretchDIBits =
            (smol_StretchDIBits_proc*)GetProcAddress(smol__wingdi, "StretchDIBits");

        smol__cursor[SMOL_CURSOR_NONE] = NULL;
        smol__cursor[SMOL_CURSOR_ARROW] = LoadCursor(NULL, IDC_ARROW);
        smol__cursor[SMOL_CURSOR_IBEAM] = LoadCursor(NULL, IDC_IBEAM);
        smol__cursor[SMOL_CURSOR_HAND] = LoadCursor(NULL, IDC_HAND);
        smol__cursor[SMOL_CURSOR_CROSS] = LoadCursor(NULL, IDC_CROSS);
        smol__cursor[SMOL_CURSOR_NO] = LoadCursor(NULL, IDC_NO);
        smol__cursor[SMOL_CURSOR_GRAB_X] = LoadCursor(NULL, IDC_SIZEWE);
        smol__cursor[SMOL_CURSOR_GRAB_Y] = LoadCursor(NULL, IDC_SIZENS);
        smol__cursor[SMOL_CURSOR_GRAB_X_ASCEND] = LoadCursor(NULL, IDC_SIZENWSE);
        smol__cursor[SMOL_CURSOR_GRAB_X_DESCEND] = LoadCursor(NULL, IDC_SIZENESW);
        smol__cursor[SMOL_CURSOR_GRAB_XY] = LoadCursor(NULL, IDC_SIZEALL);
    }

    if (smol__dwmapi == NULL) {
        smol__dwmapi = LoadLibrary(TEXT("Dwmapi.dll"));
        smol_DwmFlush = (smol_DwmFlush_proc*)GetProcAddress(smol__dwmapi, "DwmFlush");
    }

#ifdef SMOL_FRAME_MULTITHREADED
    if (!smol__frame_thread) {
        smol__frame_thread_started = CreateEvent(NULL, TRUE, FALSE, NULL);
        smol__frame_thread_should_exit = CreateEvent(NULL, FALSE, FALSE, NULL);
        smol__frame_thread = CreateThread(NULL, 4096, smol_frame_thread, NULL, 0, NULL);
        WaitForSingleObject(smol__frame_thread_started, INFINITE);
    }
#endif
    ...
    HWND wnd = NULL;
    POINT point = {0};
    DWORD frame_style = WS_VISIBLE | WS_SYSMENU | WS_MINIMIZEBOX;
    DWORD frame_style_ex = 0;
    smol_frame_t* result = NULL;

#ifndef SMOL_FRAME_MULTITHREADED
    ...
    if (GetCursorPos(&point) == TRUE) {
        ...
        if (GetMonitorInfo(monitor, &monitorInfo) != FALSE) {
            win_rect.left =
                ((monitorInfo.rcMonitor.left + monitorInfo.rcMonitor.right) - config->width) >> 1;
            win_rect.top =
                ((monitorInfo.rcMonitor.top + monitorInfo.rcMonitor.bottom) - config->height) >> 1;
            win_rect.right = win_rect.left + config->width;
            win_rect.bottom = win_rect.top + config->height;
        }
    }

    if (config->flags & SMOL_FRAME_CONFIG_HAS_TITLEBAR)
        frame_style |= WS_CAPTION;
    if (config->flags & SMOL_FRAME_CONFIG_HAS_MAXIMIZE_BUTTON)
        frame_style |= WS_MAXIMIZEBOX;
    if (config->flags & SMOL_FRAME_CONFIG_IS_RESIZABLE)
        frame_style |= WS_SIZEBOX;
    if (config->flags & SMOL_FRAME_CONFIG_BORDERLESS)
        frame_style &= ~WS_BORDER;
    if (config->flags & SMOL_FRAME_CONFIG_ALWAYS_ON_TOP)
        frame_style_ex |= WS_EX_TOPMOST;
    AdjustWindowRect(&win_rect, frame_style, FALSE);
    ...
    if (config->gl_spec) {
        ...
        int has_gl_arb_functionality =
            (smol_wglChoosePixelFormatARB && smol_wglCreateContextAttribsARB &&
             smol_wglGetPixelFormatAttribfvARB && smol_wglGetPixelFormatAttribivARB);

        if (!has_gl_arb_functionality) {
            HMODULE gl_module = LoadLibrary(TEXT("opengl32.dll"));

            smol_wglCreateContext =
                (smol_wglCreateContext_proc*)GetProcAddress(gl_module, "wglCreateContext");
            smol_wglDeleteContext =
                (smol_wglDeleteContext_proc*)GetProcAddress(gl_module, "wglDeleteContext");
            smol_wglMakeCurrent =
                (smol_wglMakeCurrent_proc*)GetProcAddress(gl_module, "wglMakeCurrent");
            smol_wglGetProcAddress =
                (smol_wglGetProcAddress_proc*)GetProcAddress(gl_module, "wglGetProcAddress");
            ...
            pfd.nVersion = 1;
            pfd.dwFlags =
                PFD_SUPPORT_OPENGL | PFD_DRAW_TO_WINDOW | PFD_DOUBLEBUFFER | PFD_SWAP_EXCHANGE;
            ...
            smol_wglCreateContextAttribsARB = (smol_wglCreateContextAttribsARB_proc*)
                smol_wglGetProcAddress("wglCreateContextAttribsARB");
            smol_wglChoosePixelFormatARB =
                (smol_wglChoosePixelFormatARB_proc*)smol_wglGetProcAddress("wglChoosePixelFormatARB"
                );
            smol_wglGetPixelFormatAttribfvARB = (smol_wglGetPixelFormatAttribfvARB_proc*)
                smol_wglGetProcAddress("wglGetPixelFormatAttribfvARB");
            smol_wglGetPixelFormatAttribivARB = (smol_wglGetPixelFormatAttribivARB_proc*)
                smol_wglGetProcAddress("wglGetPixelFormatAttribivARB");
            smol_wglSwapIntervalEXT =
                (smol_wglSwapIntervalEXT_proc*)smol_wglGetProcAddress("wglSwapIntervalEXT");
            has_gl_arb_functionality =
                (smol_wglChoosePixelFormatARB && smol_wglCreateContextAttribsARB &&
                 smol_wglGetPixelFormatAttribfvARB && smol_wglGetPixelFormatAttribivARB);
        ...
        if (has_gl_arb_functionality) {
            ...
            int pixelformat_atribs[64] = {
                ...
                gl_spec->stencil_bits,
                WGL_SAMPLE_BUFFERS_ARB,
                gl_spec->has_multi_sampling
            };
            ...
            BOOL res = smol_wglChoosePixelFormatARB(
                dc,
                pixelformat_atribs,
                NULL,
                64,
            ...
            if (res && numFormats && gl_spec->has_multi_sampling) {
                for (UINT i = 0; i < numFormats; i++) {
                    ...
                    int values[4] = {0};
                    BOOL has_feature =
                        smol_wglGetPixelFormatAttribivARB(dc, formats[i], 0, 4, ids, values);
                    ...
                    if (values[0]) {
                        ...
                        if (samples_diff == 0 && values[3] == gl_spec->depth_bits &&
...
DWORD WINAPI smol_frame_thread(LPVOID lpParameter) {
    ...
    window_class.lpszClassName = TEXT("smol_frame_root");
    window_class.lpfnWndProc = &smol_root_window_event_handler;
    ...
    ATOM registerResult = RegisterClassEx(&window_class);

    SMOL_ASSERT("Root window class initialization failed!" && registerResult);
    ...
    smol__wnd_class.style = CS_OWNDC | CS_HREDRAW | CS_VREDRAW;
    smol__wnd_class.lpfnWndProc = &smol_frame_handle_event;
    smol__wnd_class.hInstance = module;
    smol__wnd_class.cbClsExtra = 0;
    smol__wnd_class.cbWndExtra = 0;
    smol__wnd_class.hIcon = LoadIcon(NULL, IDI_WINLOGO);
    smol__wnd_class.hCursor = smol__cursor[SMOL_CURSOR_ARROW];
    smol__wnd_class.hbrBackground = (HBRUSH)smol_GetStockObject(BLACK_BRUSH);
    smol__wnd_class.lpszMenuName = NULL;
    smol__wnd_class.hIconSm = NULL;
    registerResult = RegisterClassEx(&smol__wnd_class);
    ...
    smol__window_count = 0x80000000;
    smol__root_window = CreateWindowEx(
        0,
    ...
    if (IsWindow(smol__root_window) == FALSE) {
        char buf[512] = {0};
        FormatMessageA(
        ...
        printf("Get Last Error returned: %d \"%s\"\n", GetLastError(), buf);
        SMOL_ASSERT(!"Window creation failed!");
    }

    SetEvent(smol__frame_thread_started);

    while (smol__window_count) {
        MSG msg = {0};
        BOOL ret = 0;
        while ((ret = GetMessage(&msg, NULL, 0, 0)) && smol__window_count) {
            if (ret < 0)
                break;
    ...
    DestroyWindow(smol__root_window);
    UnregisterClass(window_class.lpszClassName, GetModuleHandle(0));
    UnregisterClass(smol__wnd_class.lpszClassName, GetModuleHandle(0));
    smol__root_window = NULL;

    return 0;
}

struct smol_create_window_event_data {
    DWORD dwExStyle;
    LPCWSTR lpClassName;
    LPCWSTR lpWindowName;
    DWORD dwStyle;
...
HWND smol_CreateWindow(
    ...
    struct smol_create_window_event_data data = {
        dwExStyle,
        lpClassName,
        lpWindowName,
        dwStyle,
    ...
    return (HWND)SendMessage(smol__root_window, WM_SMOL_CREATE_WINDOW, (WPARAM)&data, 0);
}

BOOL smol_DestroyWindow(HWND wnd) {
    return SendMessage(smol__root_window, WM_SMOL_DESTROY_WINDOW, wnd, 0);
}

BOOL smol_SetWindowText(HWND wnd, LPCWSTR title) {
    return SendMessage(smol__root_window, WM_SMOL_SET_WINDOW_TITLE, wnd, title);
}

BOOL smol_IsWindow(HWND wnd) {
    return SendMessage(smol__root_window, WM_SMOL_SET_WINDOW_TITLE, wnd, 0);
}

struct smol_set_window_pos_event_data {
    HWND hWnd;
...
BOOL smol_SetWindowPos(HWND hWnd, HWND hWndInsertAfter, int x, int y, int cx, int cy, UINT flags) {
    struct smol_set_window_pos_event_data info = {hWnd, hWndInsertAfter, x, y, cx, cy, flags};
    return (BOOL)SendMessage(smol__root_window, WM_SMOL_SET_WINDOW_TITLE, &info, 0);
}

LRESULT CALLBACK smol_root_window_event_handler(HWND wnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_SMOL_CREATE_WINDOW: {
            struct smol_create_window_event_data* ptr = wParam;
            ...
            if (smol__window_count == 0x80000000)
                smol__window_count = 1;
            else
                smol__window_count++;
            return res;
        } break;
        case WM_SMOL_SET_WINDOW_TITLE: {
            HWND target = wParam;
            LPCWSTR title = lParam;
            return (LRESULT)SetWindowText(target, title);
        } break;
        case WM_SMOL_DESTROY_WINDOW: {
            HWND target = (HWND)wParam;
            LRESULT res = (LRESULT)DestroyWindow(target);
            ;
            smol__window_count--;
            return res;
        } break;
        case WM_SMOL_IS_WINDOW: {
            HWND target = (HWND)wParam;
            return (LRESULT)IsWindow(target);
        }
        case WM_SMOL_SET_WINDOW_POS: {
            struct smol_set_window_pos_event_data* ptr = wParam;
            return (LRESULT)SetWindowPos(
                ptr->hWnd,
                ptr->hWndInsertAfter,
                ptr->x,
                ptr->y,
...
void smol_frame_set_default_cursor(smol_frame_t* frame, unsigned int cursor_id) {
    SetCursor(smol__cursor[cursor_id]);
}
...
void smol_frame_set_title(smol_frame_t* frame, const char* title) {
    ...
    SMOL_ASSERT(
        "Failed to set window title!" && smol_SetWindowText(frame->frame_handle_win32, wide_title)
    );
}

void smol_frame_set_position(smol_frame_t* frame, int x, int y) {
    smol_SetWindowPos(
        frame->frame_handle_win32,
        HWND_TOPMOST,
        x,
        y,
        0,
...
void smol_frame_destroy(smol_frame_t* frame) {
    ...
    SMOL_ASSERT(smol_DestroyWindow(frame->frame_handle_win32));
#ifdef SMOL_FRAME_MULTITHREADED
    if (smol__window_count == 0) {
        WaitForSingleObject(smol__frame_thread, INFINITE);
    }
#endif
...
int smol_frame_gl_swap_buffers(smol_frame_t* frame) {
    if (!frame->gl.context)
        return 0;
    int result = smol_SwapBuffers(GetDC(frame->frame_handle_win32));
    ;
    smol_DwmFlush();
    return result;
}
    ...
    switch (msg) {
        ...
        case WM_XBUTTONDOWN: {
            ...
            switch (msg) {
                ...
                case WM_MBUTTONDOWN:
                    ...
                    break;
                case WM_XBUTTONDOWN:
                    event.mouse.button = 4 + HIWORD(wParam);
                    break;
            }
        ...
        case WM_CHAR: {
            ...
            if (surrogate == 0xD800) {
                ...
                chr = ((unsigned int)frame->high_utf16_surrogate << 10) |
                      ((unsigned int)(wParam) & 0x3FF);
    ...
    switch (key) {
        ...
        case VK_RETURN:
            if ((ext >> 6) & 1)
                ...
                return SMOLK_RETURN;
        case VK_BACK:
            return SMOLK_BACKSPACE;
        case VK_SPACE:
            return SMOLK_SPACE;
        case VK_CONTROL:
            if ((ext >> 6) & 1)
                return SMOLK_RCONTROL;
            else
                return SMOLK_LCONTROL;
        case VK_LCONTROL:
            return SMOLK_LCONTROL;
        case VK_RCONTROL:
            return SMOLK_RCONTROL;
        ...
        case VK_TAB:
            return SMOLK_TAB;
        case VK_CAPITAL:
            return SMOLK_CAPSLOCK;
        case VK_NUMLOCK:
            return SMOLK_NUMLOCK;
        case VK_SCROLL:
            return SMOLK_SCROLLLOCK;
        case VK_SNAPSHOT:
            return SMOLK_PRINTSCREEN;
        case VK_PAUSE:
            return SMOLK_PAUSE;
        case VK_INSERT:
            return SMOLK_INSERT;
        ...
        case 0X39:
            return SMOLK_NUM9;
        case VK_NUMPAD0:
            return SMOLK_NUMPAD0;
        case VK_NUMPAD1:
            return SMOLK_NUMPAD1;
        case VK_NUMPAD2:
            return SMOLK_NUMPAD2;
        case VK_NUMPAD3:
            return SMOLK_NUMPAD3;
        case VK_NUMPAD4:
            return SMOLK_NUMPAD4;
        case VK_NUMPAD5:
            return SMOLK_NUMPAD5;
        case VK_NUMPAD6:
            return SMOLK_NUMPAD6;
        case VK_NUMPAD7:
            return SMOLK_NUMPAD7;
        case VK_NUMPAD8:
            return SMOLK_NUMPAD8;
        case VK_NUMPAD9:
            return SMOLK_NUMPAD9;
        case VK_F1:
            return SMOLK_F1;
        case VK_F2:
            return SMOLK_F2;
        ...
        case VK_ADD:
            return SMOLK_ADD;
        case VK_SUBTRACT:
            return SMOLK_SUBSTRACT;
        case VK_MULTIPLY:
            return SMOLK_ASTERISK;
        case VK_DIVIDE:
            return SMOLK_DIVIDE;
        case VK_OEM_PLUS:
            return SMOLK_PLUS;
...
void smol_frame_blit_pixels(
    ...
    smol_StretchDIBits(
        GetDC(frame->frame_handle_win32),
        dstX,
        dstY,
        dstW,
        dstH,
...
Atom smol__wm_state_atom;
Atom smol__vmax_atom;
Atom smol__hmax_atom;
Atom smol__frame_handle_atom;
Atom smol__change_motif;
...
typedef GLXContext(APIENTRYP PFNGLXCREATECONTEXTPROC)(
    Display* dpy,
    XVisualInfo* vis,
...
typedef void(APIENTRYP PFNGLXCOPYCONTEXTPROC)(
    Display* dpy,
    GLXContext src,
    GLXContext dst,
    unsigned long mask
...
typedef int(APIENTRYP
                PFNGLXGETCONFIGPROC)(Display* dpy, XVisualInfo* visual, int attrib, int* value);
...
typedef GLXFBConfig*(APIENTRYP PFNGLXCHOOSEFBCONFIGPROC)(
    Display* dpy,
    int screen,
    const int* attrib_list,
    int* nelements
);
typedef int(APIENTRYP PFNGLXGETFBCONFIGATTRIBPROC)(
    Display* dpy,
    GLXFBConfig config,
    int attribute,
    int* value
...
typedef GLXWindow(APIENTRYP PFNGLXCREATEWINDOWPROC)(
    Display* dpy,
    GLXFBConfig config,
    Window win,
    const int* attrib_list
...
typedef GLXPixmap(APIENTRYP PFNGLXCREATEPIXMAPPROC)(
    Display* dpy,
    GLXFBConfig config,
    Pixmap pixmap,
    const int* attrib_list
...
typedef GLXPbuffer(APIENTRYP PFNGLXCREATEPBUFFERPROC)(
    Display* dpy,
    GLXFBConfig config,
    const int* attrib_list
);
typedef void(APIENTRYP PFNGLXDESTROYPBUFFERPROC)(Display* dpy, GLXPbuffer pbuf);
typedef void(APIENTRYP PFNGLXQUERYDRAWABLEPROC)(
    Display* dpy,
    GLXDrawable draw,
    int attribute,
    unsigned int* value
);
typedef GLXContext(APIENTRYP PFNGLXCREATENEWCONTEXTPROC)(
    Display* dpy,
    GLXFBConfig config,
    int render_type,
...
typedef Bool(APIENTRYP PFNGLXMAKECONTEXTCURRENTPROC)(
    Display* dpy,
    GLXDrawable draw,
    GLXDrawable read,
    GLXContext ctx
...
typedef int(APIENTRYP
                PFNGLXQUERYCONTEXTPROC)(Display* dpy, GLXContext ctx, int attribute, int* value);
typedef void(APIENTRYP
                 PFNGLXSELECTEVENTPROC)(Display* dpy, GLXDrawable draw, unsigned long event_mask);
...
typedef int smol_XGetWindowAttributes_proc(
    Display* display,
    Window parentWindow,
    XWindowAttributes* attirbutes
);
...
typedef Status
smol_XSetWMProtocols_proc(Display* display, Window window, Atom* protocols, int count);
typedef int smol_XStoreName_proc(Display* display, Window window, _Xconst char* window_name);
typedef XIM
smol_XOpenIM_proc(Display* display, struct _XrmHashBucketRec* rdb, char* res_name, char* res_class);
...
typedef GC smol_XCreateGC_proc(
    Display* display,
    Drawable drawable,
...
typedef int smol_XChangeProperty_proc(
    Display* display,
    Window window,
    Atom property,
    Atom type,
...
typedef Status smol_XSendEvent_proc(
    Display* display,
    Window w,
    Bool propagate,
    long event_mask,
...
typedef int smol_XPutImage_proc(
    Display* display,
    Drawable d,
    GC gc,
    XImage* image,
...
typedef Pixmap smol_XCreatePixmap_proc(
    Display* display,
    Drawable d,
    unsigned int width,
    unsigned int height,
...
typedef Cursor smol_XCreatePixmapCursor_proc(
    Display* display,
    Pixmap source,
    Pixmap mask,
...
typedef void smol_XWarpPointer_proc(
    Display* display,
    Window src_w,
    Window dst_w,
    int src_x,
...
static smol_XSendEvent_proc* smol_XSendEvent;
...
static smol_XWarpPointer_proc* smol_XWarpPointer;

typedef XVisualInfo* smol_glXGetVisualFromFBConfig_proc(Display* display, GLXFBConfig window);
typedef void smol_glXMakeCurrent_proc(Display* display, Window window, GLXContext context);
typedef GLXFBConfig*
smol_glXChooseFBConfig_proc(Display* display, int screen, const int* attribs, int* result);
typedef void* smol_glXGetProcAddress_proc(unsigned char* proc);
typedef int
smol_glXGetFBConfigAttrib_proc(Display* display, GLXFBConfig config, int attribute, int* value);
typedef void smol_glXSwapBuffers_proc(Display* display, Window window);
typedef GLXContext smol_glXCreateContextAttribsARB_proc(
    Display* dpy,
    GLXFBConfig config,
    ...
    if (frame->renderer) {
        ...
        for (int y = 0; y < renderer->height; y++) {
            ...
            for (int x = 0; x < renderer->width; x++) {
                ...
                renderer->pixel_data[dstX + dstY * renderer->width] =
                    frame->renderer->pixel_data[srcX + srcY * frame->renderer->width];
            }
        }
        ...
        renderer->gc =
            smol_XCreateGC(frame->display_server_connection, frame->frame_window, 0, NULL);
    }
...
} smol_motif_wm_hints_t;

#define SMOL__MWM_HINTS_DECORATIONS (1L << 1)
...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
    ...
    if (!smol__X11_so) {
        if (!smol__X11_so)
            smol__X11_so = dlopen("libX11.so", RTLD_NOW);

#define GET_SYM(sym) (*((void**)&smol_##sym) = dlsym(smol__X11_so, #sym))

        GET_SYM(XOpenDisplay);
        GET_SYM(XCloseDisplay);
        GET_SYM(XGetWindowAttributes);
        GET_SYM(XCreateWindow);
        GET_SYM(XDestroyWindow);
        GET_SYM(XMapWindow);
        GET_SYM(XFlush);
        GET_SYM(XSetWMNormalHints);
        GET_SYM(XInternAtom);
        GET_SYM(XSetWMProtocols);
        GET_SYM(XStoreName);
        GET_SYM(XOpenIM);
        GET_SYM(XCreateImage);
        GET_SYM(XPutImage);
        GET_SYM(XCreateGC);
        GET_SYM(XFreeGC);
        GET_SYM(XOpenDisplay);
        GET_SYM(XGetWindowAttributes);
        GET_SYM(XCreateWindow);
        GET_SYM(XAllocSizeHints);
        GET_SYM(XInternAtom);
        GET_SYM(XSetWMProtocols);
        GET_SYM(XStoreName);
        GET_SYM(XMapWindow);
        GET_SYM(XOpenIM);
        GET_SYM(XGetIMValues);
        GET_SYM(XCreateIC);
        GET_SYM(XSetICFocus);
        GET_SYM(XFlush);
        GET_SYM(XChangeProperty);
        GET_SYM(XFree);
        GET_SYM(XCreateColormap);
        GET_SYM(XInstallColormap);
        GET_SYM(XStoreName);
        GET_SYM(XDestroyWindow);
        GET_SYM(XDestroyIC);
        GET_SYM(XCloseIM);
        GET_SYM(XCloseDisplay);
        GET_SYM(XNextEvent);
        GET_SYM(XFilterEvent);
        GET_SYM(XPeekEvent);
        GET_SYM(XLookupKeysym);
        GET_SYM(Xutf8LookupString);
        GET_SYM(XPending);
        GET_SYM(XCreatePixmap);
        GET_SYM(XFreePixmap);
        GET_SYM(XCreatePixmapCursor);
        GET_SYM(XFreeCursor);
        GET_SYM(XDefineCursor);
        GET_SYM(XUndefineCursor);
        GET_SYM(XWarpPointer);
        GET_SYM(XSendEvent);
#undef GET_SYM
    }
    ...
    if (smol__change_motif == None) {
        smol__change_motif = smol_XInternAtom(display, "_MOTIF_WM_HINTS", False);
    }
    ...
    if (smol__wm_state_atom == None) {
        smol__wm_state_atom = smol_XInternAtom(display, "_NET_WM_STATE", False);
    }

    if (smol__vmax_atom == None) {
        smol__vmax_atom = smol_XInternAtom(display, "_NET_WM_STATE_MAXIMIZED_VERT", False);
    }

    if (smol__hmax_atom == None) {
        smol__hmax_atom = smol_XInternAtom(display, "_NET_WM_STATE_MAXIMIZED_HORZ", False);
        ;
    }
    ...
    if (!(config->flags & SMOL_FRAME_CONFIG_HAS_TITLEBAR)) {
        smol_motif_wm_hints_t hints;
        hints.flags = SMOL__MWM_HINTS_DECORATIONS;
        hints.decorations = 0;
        status = smol_XChangeProperty(
            display,
            result_window,
            smol__change_motif,
            smol__change_motif,
    ...
    result->im = im;
    result->renderer = NULL;
    result->config = config->flags;

    smol_XChangeProperty(
        display,
        result_window,
    ...
    if (!(config->gl_spec)) {
        ...
        smol_frame_gl_spec_t* spec = (smol_frame_gl_spec_t*)config->gl_spec;

        int initialized =
            (smol_glXGetProcAddress && smol_glXGetVisualFromFBConfig && smol_glXChooseFBConfig &&
             smol_glXSwapBuffers && smol_glXCreateContextAttribsARB);
        if (!initialized) {
            ...
            smol_glXGetVisualFromFBConfig =
                (smol_glXGetVisualFromFBConfig_proc*)dlsym(libgl, "glXGetVisualFromFBConfig");
            smol_glXChooseFBConfig =
                (smol_glXChooseFBConfig_proc*)dlsym(libgl, "glXChooseFBConfig");
            smol_glXMakeCurrent = (smol_glXMakeCurrent_proc*)dlsym(libgl, "glXMakeCurrent");
            smol_glXGetFBConfigAttrib =
                (smol_glXGetFBConfigAttrib_proc*)dlsym(libgl, "glXGetFBConfigAttrib");
            smol_glXGetProcAddress =
                (smol_glXGetProcAddress_proc*)dlsym(libgl, "glXGetProcAddress");
            smol_glXSwapBuffers = (smol_glXSwapBuffers_proc*)dlsym(libgl, "glXSwapBuffers");

            smol_glXCreateContextAttribsARB = (smol_glXCreateContextAttribsARB_proc*)
                smol_glXGetProcAddress((unsigned char*)"glXCreateContextAttribsARB");
            smol_glXSwapIntervalEXT = (smol_glXSwapIntervalEXT_proc*)smol_glXGetProcAddress(
		...
		*/
        ...
        GLXFBConfig config = NULL;
        {
            int count = 0;
            GLXFBConfig* configs =
                smol_glXChooseFBConfig(display, DefaultScreen(display), attribs, &count);
            int bestFbc = -1;
        ...
        Colormap color_map =
            smol_XCreateColormap(display, root_window, visualinfo->visual, AllocNone);
        ...
        int context_attribs[] = {
            ...
            spec->is_backward_compatible ? GLX_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB
        ...
        */
        result->gl.context =
            smol_glXCreateContextAttribsARB(display, config, NULL, True, context_attribs);
        smol_glXMakeCurrent(
            result->display_server_connection,
            result->frame_window,
            result->gl.context
        );
    }
    ...
    if (!smol__frame_empty_cursor) {
        ...
        XColor dummy = {0};
        smol__frame_empty_cursor =
            smol_XCreatePixmapCursor(display, blank, blank, &dummy, &dummy, 0, 0);
    }
...
void smol_frame_set_maximized(smol_frame_t* frame, int maximize) {
    ...
    ev.window = frame->frame_window;
    ev.message_type = smol__wm_state_atom;
    ev.format = 32;
    ev.data.l[0] = maximize ? 1 : 0;
    ev.data.l[1] = smol__hmax_atom;
    ev.data.l[2] = smol__vmax_atom;
    ev.data.l[3] = 0;

    smol_XSendEvent(
        frame->display_server_connection,
        DefaultRootWindow(frame->display_server_connection),
        False,
...
void* smol_gl_get_proc_address(const char* func) {
    ...
    return smol_glXGetProcAddress((unsigned char*)func);
}
...
VkSurfaceKHR smol_frame_create_vulkan_surface(
    ...
    ci.dpy = frame->display_server_connection;
    ci.window = frame->frame_window;
    VkResult status = vkCreateXlibSurfaceKHR(instance, &ci, alloc_cb, &surface_khr);
...
void smol_frame_destroy(smol_frame_t* frame) {
    ...
    if (frame->config & SMOL_FRAME_CONFIG_OWNS_EVENT_QUEUE)
        smol_event_queue_destroy(&frame->event_queue);
    if (frame->renderer)
...
void smol_frame_set_cursor_visibility(smol_frame_t* frame, int visibility) {
    if (visibility != frame->cursor_hidden)
        return;
    ...
    frame->cursor_hidden = !visibility;
}
...
void smol_frame_update(smol_frame_t* frame) {
    ...
    while (smol_XPending(frame->display_server_connection)) {
        ...
        switch (xevent.type) {
            ...
            case ClientMessage: {
                //if(xevent.xclient.type == smol__wm_delete_window_atom) {
                //    puts("Should quit.");
                if ((Atom)xevent.xclient.data.l[0] == smol__wm_delete_window_atom) {
                    event.type = SMOL_FRAME_EVENT_CLOSED;
                    frame->should_close = 1;
                    smol_event_queue_push_back(frame->event_queue, &event);
                }
            ...
            case KeyRelease: {
                ...
                event.type = ((xevent.type == KeyRelease) && physical) ? SMOL_FRAME_EVENT_KEY_UP
                                                                       : SMOL_FRAME_EVENT_KEY_DOWN;
                ...
                if (event.type == SMOL_FRAME_EVENT_KEY_DOWN) {
                    char buffer[6] = {0};
                    Status status = 0;
                    int count = smol_Xutf8LookupString(
                        frame->ic,
                        &xevent.xkey,
                        buffer,
                        6,
                    ...
                    switch (count) {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            unicode = (buffer[0] & 0x0F) << 12 | (buffer[1] & 0x3F) << 6 |
                                      (buffer[2] & 0x3F);
                            break;
                        case 4:
                            unicode = (buffer[0] & 0x07) << 18 | (buffer[1] & 0x3F) << 12 |
                                      (buffer[2] & 0x3F) << 6 | (buffer[3] & 0x3F);
                            break;
                        case 5:
                            unicode = (buffer[0] & 0x03) << 24 | (buffer[1] & 0x3F) << 18 |
                                      (buffer[2] & 0x3F) << 12 | (buffer[3] & 0x3F) << 6 |
                                      (buffer[4] & 0x3F);
                            break;
                        case 6:
                            unicode = (buffer[0] & 0x01) << 30 | (buffer[1] & 0x3F) << 24 |
            ...
            case ButtonRelease: {
                if (xevent.xbutton.button < 4) {
                    event.type = xevent.type == ButtonPress ? SMOL_FRAME_EVENT_MOUSE_BUTTON_DOWN
                                                            : SMOL_FRAME_EVENT_MOUSE_BUTTON_UP;
...
void smol_frame_blit_pixels(
    ...
    SMOL_ASSERT(
        "Frame has no renderer! Frame was probably initialized with OpenGL config!" &&
        frame->renderer
    );
    ...
    Visual* visual = DefaultVisual(
        frame->display_server_connection,
        DefaultScreen(frame->display_server_connection)
    );

    unsigned int red_mul = (-visual->red_mask) & visual->red_mask;
    unsigned int green_mul = (-visual->green_mask) & visual->green_mask;
    unsigned int blue_mul = (-visual->blue_mask) & visual->blue_mask;
    ...
    for (int y = startY; y < endY; y++) {
        ...
        for (int x = startX; x < endX; x++) {
            ...
            renderer->pixel_data[dX + dY * renderer->width] =
                (((pixel >> 0x00) & 0xFF) * red_mul) | (((pixel >> 0x08) & 0xFF) * green_mul) |
...
void eg_debug_proc(
    ...
    if ((messageType == EGL_DEBUG_MSG_CRITICAL_KHR) || (messageType == EGL_DEBUG_MSG_ERROR_KHR)) {
        puts(
            (messageType == EGL_DEBUG_MSG_CRITICAL_KHR) ? "CRITICAL EGL ERROR!"
                                                        : "NON-CRITICAL EGL ERROR!"
        );
...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
    ...
    if (smol__wm_delete_window_atom == 0 && smol__wm_protocols_atom == 0) {
        ...
        xcb_intern_atom_reply_t* protocolsReply =
            xcb_intern_atom_reply(connection, protocolsCookie, NULL);
        smol__wm_protocols_atom = protocolsReply->atom;
        free(protocolsReply);

        //Delete window atom acquisition
        xcb_intern_atom_cookie_t deleteWindowCookie =
            xcb_intern_atom(connection, 0, strlen("WM_DELETE_WINDOW"), "WM_DELETE_WINDOW");
        xcb_intern_atom_reply_t* deleteWindowReply =
            xcb_intern_atom_reply(connection, deleteWindowCookie, NULL);
        ...
        xcb_change_property(
            ...
            XCB_PROP_MODE_REPLACE,
            window,
            smol__wm_protocols_atom,
            4,
            32,
            1,
            &smol__wm_delete_window_atom
        );
    ...
    kbkeymap = xkb_x11_keymap_new_from_device(
        kbcontext,
        connection,
        device_id,
...
#define EGL_ASSERT
        ...
        EGLint args[] = {EGL_PLATFORM_XCB_SCREEN_EXT, screen_id, EGL_NONE};
        ...
        cookie = xcb_create_colormap_checked(
            connection,
        ...
        EGLint context_attributes[] = {
            ...
            spec->is_backward_compatible ? EGL_CONTEXT_OPENGL_COMPATIBILITY_PROFILE_BIT
                                         : EGL_CONTEXT_OPENGL_CORE_PROFILE_BIT,
            EGL_CONTEXT_OPENGL_FORWARD_COMPATIBLE,
            spec->is_forward_compatible ? EGL_TRUE : EGL_FALSE,
            EGL_CONTEXT_OPENGL_DEBUG,
            spec->is_debug ? EGL_TRUE : EGL_FALSE,
            EGL_NONE
        };
        ...
        EGLSurface egl_surface =
            eglCreatePlatformWindowSurface(egl_display, egl_configs[best_config], &window, NULL);
        EGL_ASSERT;
...
void smol_frame_update(smol_frame_t* frame) {
    ...
    for (xcb_generic_event_t* xevent;
         (xevent = xcb_poll_for_event(frame->display_server_connection));) {
        ...
        switch (response) {
            ...
            case XCB_KEY_RELEASE: {
                ...
                xkb_state_update_key(
                    frame->kbstate,
                    ev->detail,
                    response == XCB_KEY_PRESS ? XKB_KEY_DOWN : XKB_KEY_UP
                );

                if (response == XCB_KEY_PRESS) {
                    xcb_generic_event_t* next =
                        xcb_poll_for_queued_event(frame->display_server_connection);
                    if (next && (next->response_type & 0x7F) == XCB_KEY_RELEASE) {
                        xcb_key_press_event_t* nev = (xcb_key_press_event_t*)next;
                        if (nev->response_type == XCB_KEY_RELEASE && ev->time == nev->time &&
                            ev->detail == nev->detail) {
                            physical = 0;
                        }
                    }
                }
                ...
                event.type = ((xevent->response_type & 0x7F) == XCB_KEY_PRESS && physical)
                                 ? SMOL_FRAME_EVENT_KEY_DOWN
                                 : SMOL_FRAME_EVENT_KEY_UP;
            ...
            case XCB_BUTTON_RELEASE: {
                ...
                if (ev->detail < XCB_BUTTON_INDEX_4) {
                    event.type = xevent->response_type == XCB_BUTTON_PRESS
                                     ? SMOL_FRAME_EVENT_MOUSE_BUTTON_DOWN
                                     : SMOL_FRAME_EVENT_MOUSE_BUTTON_UP;
                    ...
                    if (ev->detail == XCB_BUTTON_INDEX_4)
                        delta = +1;
                    if (ev->detail == XCB_BUTTON_INDEX_5)
                        delta = -1;
...
#pragma region Web Implementation
...
EM_BOOL smol_fullscreen_event(
    int event_type,
    const EmscriptenFullscreenChangeEvent* fullscreen_event,
    void* user_data
);
EM_BOOL smol_resize_event(int event_type, const EmscriptenUiEvent* ui_event, void* user_data);
...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
    ...
    *frame = (smol_frame_t){0};
    ...
    //emscripten_set_element_css_size(frame->element, config->width, config->height);
    frame->width = config->width;
    frame->height = config->height;

    if (config->gl_spec) {
        const smol_frame_gl_spec_t* spec = config->gl_spec;
        EmscriptenWebGLContextAttributes attribs = {
            .alpha = spec->alpha_bits > 0,
            .depth = spec->depth_bits > 0,
            .stencil = spec->stencil_bits > 0,
            .antialias = spec->has_multi_sampling != 0,
            .premultipliedAlpha = 1,
            .preserveDrawingBuffer = 1,
            .powerPreference = 0,
            .failIfMajorPerformanceCaveat = 0,
            .majorVersion = 2, //spec->major_version,
            .minorVersion = 0, //spec->minor_version,
        ...
        //if(attribs.majorVersion > 2) attribs.majorVersion = 2;
    ...
    SMOL_ASSERT(
        emscripten_set_fullscreenchange_callback(
            ...
            frame,
            1,
            &smol_fullscreen_event
        ) == EMSCRIPTEN_RESULT_SUCCESS
    );
    SMOL_ASSERT(
        emscripten_set_resize_callback(frame->element, frame, 1, &smol_resize_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );

    SMOL_ASSERT(
        emscripten_set_mousemove_callback(frame->element, frame, 1, &smol_mouse_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );
    SMOL_ASSERT(
        emscripten_set_mouseup_callback(frame->element, frame, 1, &smol_mouse_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );
    SMOL_ASSERT(
        emscripten_set_mousedown_callback(frame->element, frame, 1, &smol_mouse_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );

    SMOL_ASSERT(
        emscripten_set_wheel_callback(frame->element, frame, 1, &smol_wheel_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );

    SMOL_ASSERT(
        emscripten_set_keydown_callback(frame->element, frame, 1, &smol_key_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );
    SMOL_ASSERT(
        emscripten_set_keyup_callback(frame->element, frame, 1, &smol_key_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );
    SMOL_ASSERT(
        emscripten_set_keypress_callback(frame->element, frame, 1, &smol_key_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );

    SMOL_ASSERT(
        emscripten_set_focus_callback(frame->element, frame, 1, &smol_focus_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );
    SMOL_ASSERT(
        emscripten_set_blur_callback(frame->element, frame, 1, &smol_focus_event) ==
        EMSCRIPTEN_RESULT_SUCCESS
    );
...
bool smol_canvas_resized_callback(int event_Type, const void* reserved, void* user_data) {
    smol_frame_t* frame = (smol_frame_t*)user_data;
    emscripten_get_canvas_element_size(frame->element, &frame->width, &frame->height);
    return EM_TRUE;
}
...
void maximize_deferred(void* user) {
    struct deferred_data* deferred = (struct deferred_data*)user;
    if (deferred->maximize) {
        EmscriptenFullscreenStrategy strategy = SMOL_INIT(EmscriptenFullscreenStrategy);
        emscripten_request_fullscreen_strategy(deferred->frame->element, 1, &strategy);
        strategy.canvasResizedCallback = smol_canvas_resized_callback;
        strategy.canvasResizedCallbackUserData = (void*)deferred->frame;
        strategy.filteringMode = EMSCRIPTEN_FULLSCREEN_FILTERING_NEAREST;
        strategy.canvasResolutionScaleMode = EMSCRIPTEN_FULLSCREEN_SCALE_DEFAULT;
        emscripten_enter_soft_fullscreen(deferred->frame->element, &strategy);
    } else {
...
void smol_frame_set_maximized(smol_frame_t* frame, int maximize) {
    struct deferred_data* deferred = (struct deferred_data*)malloc(sizeof(struct deferred_data));
    deferred->frame = frame;
    deferred->maximize = 1;
    emscripten_set_timeout(&maximize_deferred, 250.0, (void*)deferred);
}
...
smol_fullscreen_event(
    ...
    smol_frame_event_t ev = SMOL_INIT(smol_frame_event_t);
    ev.frame = frame;

    if (fullscreen_event->isFullscreen) {
        printf("Entered into fullscreen.\n");
        ev.size.width = fullscreen_event->screenWidth;
        ev.size.height = fullscreen_event->screenHeight;
    } else {
        printf("Escaped from fullscreen.\n");
        ev.size.width = fullscreen_event->elementHeight;
        ev.size.height = fullscreen_event->elementWidth;
    }
    ...
    switch (event_type) {
        ...
        case EMSCRIPTEN_EVENT_MOUSEUP: {
            ...
            if (event_type == EMSCRIPTEN_EVENT_MOUSEDOWN ||
                ...
                ev.type = (event_type == EMSCRIPTEN_EVENT_MOUSEDOWN)
                              ? SMOL_FRAME_EVENT_MOUSE_BUTTON_DOWN
                              : SMOL_FRAME_EVENT_MOUSE_BUTTON_UP;
...
EM_BOOL smol_key_event(int event_type, const EmscriptenKeyboardEvent* key_event, void* user_data) {
    ...
    switch (event_type) {
        ...
        case EMSCRIPTEN_EVENT_KEYUP: {
            ev.type = (event_type == EMSCRIPTEN_EVENT_KEYDOWN) ? SMOL_FRAME_EVENT_KEY_DOWN
                                                               : SMOL_FRAME_EVENT_KEY_UP;
            ...
            if (event_type == EMSCRIPTEN_EVENT_KEYDOWN) {
                ...
                unsigned int unicode = 0;
                if ((key_event->key[0] >= 'A' && key_event->key[0] <= 'Z') &&
                ...
                if ((buffer[0] & 0xFE) == 0xFE)
                    unicode = (buffer[0] & 0x01) << 0x1E | (buffer[1] & 0x3F) << 0x18 |
                              (buffer[2] & 0x3F) << 0x12 | (buffer[3] & 0x3F) << 0x0C |
                              (buffer[4] & 0x3F) << 0x06 | (buffer[5] & 0x3F);
                if ((buffer[0] & 0xFC) == 0xFC)
                    unicode = (buffer[0] & 0x03) << 0x18 | (buffer[1] & 0x3F) << 0x12 |
                              (buffer[2] & 0x3F) << 0x0C | (buffer[3] & 0x3F) << 0x06 |
                              (buffer[4] & 0x3F);
                if ((buffer[0] & 0xF0) == 0xF0)
                    unicode = (buffer[0] & 0x07) << 0x12 | (buffer[1] & 0x3F) << 0x0C |
                              (buffer[2] & 0x3F) << 0x06 | (buffer[3] & 0x3F);
                if ((buffer[0] & 0xE0) == 0xE0)
                    unicode = (buffer[0] & 0x0F) << 0x0C | (buffer[1] & 0x3F) << 0x06 |
                              (buffer[2] & 0x3F);
                if ((buffer[0] & 0xC0) == 0xC0)
                    unicode = (buffer[0] & 0x1F) << 0x06 | (buffer[1] & 0x3F);
                if ((buffer[0] & 0x7F) != 0x00)
                    unicode = (buffer[0] & 0x7F);
...
EM_BOOL smol_wheel_event(int event_type, const EmscriptenWheelEvent* wheel_event, void* user_data) {
    ...
    if (wheel_event->deltaX)
        delX = (wheel_event->deltaX < 0.) ? -1 : 1;
    if (wheel_event->deltaY)
        delY = (wheel_event->deltaX < 0.) ? -1 : 1;
...
EM_BOOL smol_focus_event(int event_type, const EmscriptenFocusEvent* focus_event, void* user_data) {
    ...
    switch (event_type) {
        case EMSCRIPTEN_EVENT_FOCUS:
            ev.type = SMOL_FRAME_EVENT_FOCUS_GAINED;
            break;
        case EMSCRIPTEN_EVENT_BLUR:
            ev.type = SMOL_FRAME_EVENT_FOCUS_LOST;
            break;
        default:
            return 0;
    }
    ...
    EM_ASM(
        {
            ...
            imageData.data.set(
                new Uint8ClampedArray(Module.HEAPU8.buffer, $1 + ($8 + $9 * $2) * 4, $10 * $11 * 4)
            );

            Module.ctx2D.putImageData(imageData, $4, $5, 0, 0, $6, $7);
            Module.ctx2D.drawImage(
                Module.canvas,
                0,
                0,
                xscale_fact * Module.canvas.width,
                yscale_fact * Module.canvas.height
            );
        },
        frame->element,
        pixBuf,
...
#if (defined(SMOL_FRAME_BACKEND_XCB) || defined(SMOL_FRAME_BACKEND_X11)) &&                        \
    defined(SMOL_PLATFORM_LINUX)
int smol_frame_mapkey(int key) {
    switch (key) {
        case XK_A:
        case XK_a:
