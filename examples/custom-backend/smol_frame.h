...
#if __cplusplus > 0 
#define SMOL_INIT(type) type{}
#define SMOL_OBJ(type) type
#else 
#define SMOL_INIT(type) (type){ 0 }
#define SMOL_OBJ(type) (type)
#endif 
...
VkSurfaceKHR smol_frame_create_vulkan_surface(smol_frame_t* frame, VkInstance instance, const VkAllocationCallbacks * alloc_cb, VkResult* result);
#endif 
...
#define SMOL_STD_ALLOCATOR SMOL_OBJ(smol_allocator_t) { smol_std_alloc, smol_std_realloc, smol_std_release, NULL }
#endif 
...
void* smol_def_alloc(smol_size_t size, void* user) { (void)user; return malloc(size); }
void* smol_def_realloc(void* mem, smol_size_t size, void* user) { (void)user; return realloc(mem, size); }
void  smol_def_release(void* mem, void* user) { free(mem); }
void* smol_allocator_alloc(smol_allocator_t* alloc, smol_size_t size) { return alloc->allocate(size, alloc->user); }
void* smol_allocator_realloc(smol_allocator_t* alloc, void* mem, smol_size_t size) { return alloc->reallocate(mem, size, alloc->user); }
void smol_allocator_free(smol_allocator_t* alloc, void* mem) { alloc->release(alloc, mem); }
...
HWND smol_CreateWindow(DWORD dwExStyle, LPCWSTR lpClassName, LPCWSTR lpWindowName, DWORD dwStyle, int X, int Y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam);
...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
	...
	if(config->flags & SMOL_FRAME_CONFIG_HAS_TITLEBAR) frame_style |= WS_CAPTION;
	if(config->flags & SMOL_FRAME_CONFIG_HAS_MAXIMIZE_BUTTON) frame_style |= WS_MAXIMIZEBOX;
	if(config->flags & SMOL_FRAME_CONFIG_IS_RESIZABLE) frame_style |= WS_SIZEBOX;
	if(config->flags & SMOL_FRAME_CONFIG_BORDERLESS) frame_style &= ~WS_BORDER;
	if(config->flags & SMOL_FRAME_CONFIG_ALWAYS_ON_TOP) frame_style_ex |= WS_EX_TOPMOST;
...
DWORD WINAPI smol_frame_thread(LPVOID lpParameter) {
	...
	smol__root_window = CreateWindowEx(0, window_class.lpszClassName, NULL, 0, 0, 0, 0, 0, NULL, NULL, window_class.hInstance, NULL);
	
	if(IsWindow(smol__root_window) == FALSE) {
		char buf[512] = { 0 };
		FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM,NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), buf, 512, NULL);
...
BOOL smol_SetWindowPos(HWND hWnd, HWND hWndInsertAfter, int x, int y, int cx, int cy, UINT flags) {
	struct smol_set_window_pos_event_data info = {
		hWnd, 
		hWndInsertAfter,
		x, 
		y,
...
void smol_frame_set_title(smol_frame_t* frame, const char* title) {
	...
	SMOL_ASSERT("Failed to set window title!" && smol_SetWindowText(frame->frame_handle_win32, wide_title));
	
}

void smol_frame_set_position(smol_frame_t* frame, int x, int y) {
	smol_SetWindowPos(frame->frame_handle_win32, HWND_TOPMOST, x, y, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
}
...
typedef Status smol_XSendEvent_proc(Display* display, Window w, Bool propagate, long event_mask, XEvent* event_send);
...
typedef void smol_XWarpPointer_proc(Display* display, Window src_w, Window dst_w, int src_x, int src_y, unsigned int src_width, unsigned int src_height, int dest_x, int dest_y);
...
typedef GLXFBConfig* smol_glXChooseFBConfig_proc(Display* display, int screen, const int* attribs, int* result);
...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
	...
	if(!(config->flags & SMOL_FRAME_CONFIG_HAS_TITLEBAR)) {
		...
		hints.decorations = 0;	
		status = smol_XChangeProperty(display, result_window, smol__change_motif, smol__change_motif, 32, PropModeReplace, (unsigned char *)&hints, sizeof(hints) / sizeof(long));
	}
	SMOL_ASSERT("Failed to set Window protocols!" && (status != 0));
	...
	if(!(config->gl_spec)) {
		...
		if(!initialized) {
			...
			smol_glXCreateContextAttribsARB = (smol_glXCreateContextAttribsARB_proc*)smol_glXGetProcAddress((unsigned char*)"glXCreateContextAttribsARB");
			smol_glXSwapIntervalEXT = (smol_glXSwapIntervalEXT_proc*)smol_glXGetProcAddress((unsigned char*)"glXSwapIntervalEXT");
...
void smol_frame_set_maximized(smol_frame_t* frame, int maximize) {
	...
	smol_XSendEvent(frame->display_server_connection, DefaultRootWindow(frame->display_server_connection), False, SubstructureRedirectMask | SubstructureNotifyMask, (XEvent *)&ev);
...
void smol_frame_destroy(smol_frame_t* frame) {
	...
	if(frame->config & SMOL_FRAME_CONFIG_OWNS_EVENT_QUEUE)smol_event_queue_destroy(&frame->event_queue);
...
void smol_frame_set_cursor_position(smol_frame_t* frame, int x, int y) {
	smol_XWarpPointer(frame->display_server_connection, None, frame->frame_window, 0, 0, 0, 0, x, y);
}
...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
	...
	*frame = (smol_frame_t){ 0 };
	...
	SMOL_ASSERT(emscripten_set_resize_callback(frame->element, frame, 1, &smol_resize_event) == EMSCRIPTEN_RESULT_SUCCESS);
...
EM_BOOL smol_resize_event(int event_type, const EmscriptenUiEvent* ui_event, void *user_data) {
	smol_frame_t* frame = (smol_frame_t*)user_data;
	emscripten_set_canvas_element_size(frame->element, ui_event->windowInnerWidth, ui_event->windowInnerHeight);
	return EM_TRUE;
}
...
EM_BOOL smol_key_event(int event_type, const EmscriptenKeyboardEvent* key_event, void *user_data) {
	...
	if(event_type == EMSCRIPTEN_EVENT_KEYDOWN) printf("Key down event.\n");
	else if(event_type == EMSCRIPTEN_EVENT_KEYUP) printf("Key up event.\n");
