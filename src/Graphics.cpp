...
namespace gui {
    ...
    void Graphics::StyledTextBegin(Json style) {
        ...
        ParseColor(style["color"], color[0], color[1], color[2], color[3]);
    ...
    TextExtents Graphics::MeasureText(const std::string& text) {
        ...
        return TextExtents{
            ...
            static_cast<float>(extents.y_advance)
        };
    }

    FontExtents Graphics::GetFontExtents() {
        cairo_font_extents_t extents;
        cairo_font_extents(Ctx(), &extents);
        return FontExtents{
            static_cast<float>(extents.ascent),
            static_cast<float>(extents.descent),
            static_cast<float>(extents.height)
        };
    }

    void Graphics::Font(FontStyle style, const std::string& font, double fontSize) {
        cairo_font_slant_t slant = CAIRO_FONT_SLANT_NORMAL;
        cairo_font_weight_t weight = CAIRO_FONT_WEIGHT_NORMAL;
        switch (style) {
    ...
    Color Color::FromStyle(Json style) {
        Color out{};
        if (style.is_array()) {
            out.r = style[0].get<float>();
            out.g = style[1].get<float>();
            out.b = style[2].get<float>();
            if (style.size() > 3)
